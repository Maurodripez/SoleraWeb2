  ...
  $.ajax({
    ...
    success: function (result) {
      ...
      for (let i = 0; i < sinDiagonal.length - 1; i++) {
        ...
        let nombre = `<td class='tablaActual'>${sinComas[2]}</td>`;
        let password = `<td class='tablaActual'>${sinComas[3]}</td>`;
        let privilegio = `<td class='tablaActual'>${sinComas[4]}</td>`;
...
function ActualizarUsuario(getId) {
  $.ajax({
    ...
    success: function (result) {
      ...
      document.getElementById("nombre").value = sinComas[1];
      document.getElementById("usuario").value = sinComas[2];
      document.getElementById("password").value = sinComas[3];
      document.getElementById("privilegio").value = sinComas[4];
