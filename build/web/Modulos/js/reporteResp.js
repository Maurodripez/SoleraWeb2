    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            for (let i = 0; i < cont - 1; i += 2) {
                ...
                } else if (sinComas[i + 1] < 20 && sinComas[i + 1] >= 8) {
                    sinEspacios = sinComas[i].replace(/\s+/g, "");
                    document
                        .getElementById(sinEspacios.toLowerCase())
                        .setAttribute("fill", "#FDCB00");
                } else if (sinComas[i + 1] >= 20) {
                    sinEspacios = sinComas[i].replace(/\s+/g, "");
                    document
                        .getElementById(sinEspacios.toLowerCase())
                        .setAttribute("fill", "#FD2F03");
                }
            }
        },
    });
        ...
        success: function (result) {
            ...
            for (let i = 1; i < sinComas.length; i = i + 2) {
                if (sinComas[i - 1] > 0) {
                    ...
                    let decimal = (sinComas[i - 1] * 100) / sinComas[sinComas.length - 1] - Math.floor((sinComas[i - 1] * 100) / sinComas[sinComas.length - 1]);
                    if (decimal >= .5) {
                        sinComas[i - 1] = Math.ceil((sinComas[i - 1] * 100) / sinComas[sinComas.length - 1]);
                    } else {
                        sinComas[i - 1] = Math.floor((sinComas[i - 1] * 100) / sinComas[sinComas.length - 1]);
                    }
                }
                switch (sinComas[i]) {
                    case "Nuevo":
                        $("#nuevoPorcentaje").html(sinComas[i - 1] + "%");
                        $("#nuevoTotal").html("Casos: " + cantSiniestro);
                        break;
                    case "En seguimiento":
                        $("#seguimiento").html(sinComas[i - 1] + "%");
                        $("#seguimientoTotal").html("Casos: " + cantSiniestro);
                        break;
                    case "Cancelado":
                        ...
                        break;
                    case "Terminado":
                        $("#terminadoPorcentaje").html(sinComas[i - 1] + "%");
                        $("#terminadoTotal").html("Casos: " + cantSiniestro);
                        break;
                }
            }
        },
            ...
            for (let i = 1; i < sinComas.length; i = i + 2) {
                if (sinComas[i - 1] > 0) {
                    ...
                    if (decimal >= .5) {
                        ...
                        porcentaje = Math.floor((sinComas[i - 1] * 100) / sinComas[sinComas.length - 1]);
                    }
                }
                switch (sinComas[i]) {
                    case "Nuevo":
                        ...
                        break;
                    case "En seguimiento":
                        seguimientoC = sinComas[i - 1];
                        $("#proceso").html("Casos: " + seguimientoC);
                        seguimientoP = sinComas[i - 1] + "%";
                        break;
                    case "Cancelado":
                        canceladoC = sinComas[i - 1];
                        $("#canceladoTotal").html("Casos: " + canceladoC);
                        canceladoP = sinComas[i - 1] + "%";
                        break;
                    case "Terminado":
                        terminadoC = sinComas[i - 1];
                        $("#terminado").html("Casos: " + terminadoC);
                        terminadoP = sinComas[i - 1] + "%";
                        break;
                }
            }
            let options = {
                ...
                series: [
                    {
                        name: "Folios",
                        data: [nuevoC, seguimientoC, canceladoC, terminadoC],
                    },
                ],
                labels: [
                    "Nuevo: " + nuevoP + "",
                    "En seguimiento: " + seguimientoP + "",
                    "Cancelado: " + canceladoP + "",
                    "Terminado: " + terminadoP + "",
                ],
                title: {
        ...
        success: function (result) {
            ...
            for (let i = 1; i < sinComas.length; i = i + 2) {
                ...
                switch (sinComas[i]) {
                    ...
                    case "TERMINADO POR PROCESO COMPLETO":
                        ...
                        tDocsP = porcentaje + "%";
                        break;
                    case "CON CONTACTO SIN COOPERACION DEL CLIENTE":
                        sinCopC = sinComas[i - 1];
                        sinCopP = porcentaje + "%";
                        break;
            ...
            let options = {
                ...
                labels: [
                    ...
                    "Terminado",
                    "Con contacto sin cooperacion de cliente",
                    "Concluido por otras vias",
                ],
                responsive: [
                    {
                        breakpoint: 480,
...
function tablaReporte(result) {
    ...
    for (let i = 0; i < sinCodificado.length - 1; i++) {
        ...
        tabla.innerHTML += `<tr class='tablaActualReporte'>${usuario +
            domingo +
            lunes +
            martes +
            miercoles +
